...
def normalize_url(url):
    try:
        ...
        if "ltn.com.tw" in netloc:
            return f"ltn.com.tw{path}"
        else:
...
def fetch_ltn_world_articles_and_save(index_url, output_dir, source_label="自由時報_國際"):
    ...
    scroll_to_bottom(driver)  # 多些次數以確保載入完畢
    time.sleep(4)
    soup = BeautifulSoup(driver.page_source, "lxml")
    driver.quit()

    anchors = soup.select("a.ph.listS_h")  # 針對世界新聞的連結選擇器
    logging.info(f"🌍 國際新聞共找到 {len(anchors)} 篇文章")
    ...
    for i, a in enumerate(anchors, 1):
        ...
        if norm_url in normalized_done_urls:
            logging.info(f"   ⏩ 已處理過：{full_url}")
            continue

        logging.info(f"   {i:02d}. 擷取文章：{full_url}")
        try:
            driver = create_driver()
            driver.get(full_url)
            time.sleep(4)
            ...
            clean_title = sanitize_filename(article.title or "未命名")
    ...
    logging.info(f"✅ 完成，自由時報世界新聞共儲存 {total_articles} 篇文章")

def fetch_ltn_articles_and_save(index_url, output_dir, source_label="自由時報財經"):
    subdir = os.path.join(output_dir, source_label)
    done_file = os.path.join(subdir, "done_urls.txt")
    done_urls = load_done_urls(done_file)
    normalized_done_urls = set(normalize_url(url) for url in done_urls)
    ...
    for block in category_blocks:
        ...
        logging.info(f"  🔎 共找到 {len(anchors)} 篇文章")
    ...
    logging.info(f"✅ 完成，自由時報共儲存 {total_articles} 篇文章")
